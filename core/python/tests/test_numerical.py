...
def test_numerical():
    ...
    assert (Integer(2) + Infinity.pos()).is_inf_pos
    assert (Integer(2) - Infinity.pos()).is_inf_neg
    assert (Integer(2) * Infinity.pos()).is_inf_pos
    assert (Integer(0) * Infinity.pos()).is_nan
    ...
    assert (Rational(2, 1) + Infinity.pos()).is_inf_pos()
    assert (Rational(2, 1) - Infinity.pos()).is_inf_neg()
    assert (Rational(2, 1) * Infinity.pos()).is_inf_pos()
    assert (Rational(0, 1) * Infinity.pos()).is_nan()
    assert (Rational(2, 1) / Infinity.pos()).is_zero()
    assert (Rational(2, 1) // Infinity.pos()).is_zero()
    ...
    assert((Infinity.pos() + Integer(2)).is_inf_pos())
    assert((Infinity.pos() - Integer(2)).is_inf_pos())
    assert((Infinity.pos() * Integer(2)).is_inf_pos())
    assert((Infinity.pos() * Integer(0)).is_nan())
    assert((Infinity.pos() / Integer(2)).is_inf_pos())
    assert((Infinity.pos() // Integer(2)).is_inf_pos())
    assert((Infinity.neg() + Integer(2)).is_inf_neg())
    assert((Infinity.neg() - Integer(2)).is_inf_neg())
    assert((Infinity.neg() * Integer(2)).is_inf_neg())
    assert((Infinity.neg() * Integer(0)).is_nan())
    assert((Infinity.neg() / Integer(2)).is_inf_neg())
    assert((Infinity.neg() // Integer(-2)).is_inf_pos())

    assert((Infinity.pos() + Rational(2, 1)).is_inf_pos())
    assert((Infinity.pos() - Rational(2, 1)).is_inf_pos())
    assert((Infinity.pos() * Rational(2, 1)).is_inf_pos())
    assert (Infinity.pos() * Rational(0, 1)).is_nan()
    assert((Infinity.pos() / Rational(2, 1)).is_inf_pos())
    assert((Infinity.pos() // Rational(2, 1)).is_inf_pos())
    assert((Infinity.neg() + Rational(2, 1)).is_inf_neg())
    assert((Infinity.neg() - Rational(2, 1)).is_inf_neg())
    assert((Infinity.neg() * Rational(2, 1)).is_inf_neg())
    assert (Infinity.neg() * Rational(0, 1)).is_nan()
    assert((Infinity.neg() / Rational(2, 1)).is_inf_neg())
    assert((Infinity.neg() // Rational(-2, 1)).is_inf_pos())
