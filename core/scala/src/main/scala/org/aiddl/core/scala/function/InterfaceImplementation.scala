...
trait InterfaceImplementation {
   ...
   */
  def checkOutput(c: Container)(x: Term): Boolean =
    checkInterfaceType(c.interface(interfaceUri)(InterfaceImplementation.Output), x, c.eval)
   ...
   */
  def checkInput(c: Container)(x: Term): Boolean =
    checkInterfaceType(c.interface(interfaceUri)(InterfaceImplementation.Input), x, c.eval)
